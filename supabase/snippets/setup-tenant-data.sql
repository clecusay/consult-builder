-- ============================================================
-- Setup tenant data for all 5 local dev accounts
-- Run: psql postgresql://postgres:postgres@127.0.0.1:54322/postgres -f supabase/snippets/setup-tenant-data.sql
-- ============================================================
...
DECLARE
  ...
  -- Auth user IDs (from auth.users)
  u1_id uuid := '97314e7f-5202-4a85-81f8-95a9dd55eaa2';
  u2_id uuid := '4eabb744-c1fa-483c-bf83-78814204cc4a';
  u3_id uuid := 'cc96b245-63dc-4b9b-8bbd-370fc0daed30';
  u4_id uuid := '22a72476-5e49-4de9-aad6-6e5f877210e0';
  u5_id uuid := '9f9d4db4-f41e-4197-9c38-9b5e94b85b3f';
  ...
  -- Body region IDs (from seed.sql — global defaults)
  -- Female face
  ...
  webhook text := 'https://discord.com/api/webhooks/1485142843898069044/Re6t2PkH9vC8yOXA4EeaPKczdkcCwwJcHbpjerY1NwBz_FJDRFwWBnBNDaBD-M9g8P9_';
...
-- ============================================================
-- 3. WIDGET CONFIGS (with Discord webhook + varied colors)
-- ============================================================
INSERT INTO widget_configs (tenant_id, webhook_url, widget_mode, diagram_type, primary_color, secondary_color, accent_color) VALUES
  (t1_id, webhook, 'regions_concerns', 'face',      '#1a1a2e', '#16213e', '#0f3460'),
  (t2_id, webhook, 'regions_concerns', 'face',      '#2563eb', '#1d4ed8', '#1e40af'),
  (t3_id, webhook, 'regions_concerns', 'face',      '#ec4899', '#db2777', '#be185d'),
  (t4_id, webhook, 'regions_concerns', 'body',      '#059669', '#047857', '#065f46'),
  (t5_id, webhook, 'regions_concerns', 'full_body', '#8b5cf6', '#7c3aed', '#6d28d9');

-- ============================================================
-- 4. TENANT LOCATIONS
-- ============================================================
INSERT INTO tenant_locations (id, tenant_id, name, address, city, state, zip, phone, is_primary) VALUES
  -- Tenant 1: 2 locations
...
-- 6. SERVICES
-- ============================================================
-- Tenant 1: face-only surgical (2 services)
...
-- Tenant 2: 14 surgical face services
INSERT INTO services (id, tenant_id, category_id, name, slug, display_order) VALUES
...
-- Tenant 5: 4 mixed face+body services
INSERT INTO services (id, tenant_id, category_id, name, slug, display_order) VALUES
...
-- Tenant 3: face-only non-surgical
INSERT INTO service_body_regions (service_id, body_region_id) VALUES
...
INSERT INTO service_body_regions (service_id, body_region_id) VALUES
  ...
  -- Breast Augmentation (female only)
...
-- ============================================================
-- 9. LOCATION DISABLED SERVICES (variety)
-- ============================================================
-- Tenant 2: Disable Scar Revision at Recovery Suite
...
-- Tenant 5: Disable surgical services at East Location (injectables only)
INSERT INTO location_disabled_services (location_id, service_id) VALUES
